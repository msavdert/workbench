...
# CLAUDE_CONFIG_DIR is deliberately NOT set: it was a workaround for the
# retired devbox container's Docker volumes and it moves ~/.claude.json to
# ~/.claude/.claude.json, so a claude started from zsh (a human TTY) and one
# started from bash (ssh, Remote Control) would see different accounts and
# project state. One identity per machine: Claude Code's defaults.
