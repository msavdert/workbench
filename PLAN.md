...
mac/setup.sh`). Claude Code's `model` and `modelSettings` are seed keys now
(D8): `settings.base.json` pins `claude-fable-5-1[1m]` for a fresh install,
`/model` owns the value afterwards and `--check` no longer reports it; the
mac reports no drift. The box's 2026-09-04 skill edits (second external
auditor GLM-5.2 suspended) are committed. `motd-news.timer` is disabled by
`bootstrap.sh step_system` (it fired the script the same step had made
non-executable, 203/EXEC daily), applied with `make provision
STEPS=system`. `make maintain` then a Proxmox host reboot (kernel
7.0.14-15-pve was installed but not running; agent-vm has `onboot: 1`):
host and box back on the new kernels, KSTA=1, no failed units, hermes,
vault timers, Remote Control and agentshard units up on their own. Vault:
the nightly compile ran 2026-09-07 03:01 UTC-4, exit 0. Found on the way:
`omp-run.sh` died on the mac's bash 3.2 (empty array under `set -u`),
fixed. Later the same day, agy Remote Control on the box moved from the
2026-08-31 hand setup (deprecated `agy-daemon.sh` installer, hand-edited
wrapper with `--dangerously-skip-permissions`, hand symlink
`~/.local/bin/agy`) to agy 1.1.27's own `agy remote-control start`, which
