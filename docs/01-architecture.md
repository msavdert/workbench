...
| `~/.local/share/zsh/{completions,init}` (completion cache, fzf/starship/zoxide init snippets), `~/.local/share/zsh-plugins/` (autosuggestions, syntax-highlighting checkouts) | generated by `home/zsh/regen-completions.zsh` and `home/zsh/install-plugins.sh` | `mac/setup.sh`, `box/bootstrap.sh step_tools`; by hand `mise run completions:regen`, `mise run zsh:plugins` |
| `~/.gitconfig`, `~/.config/git/allowed_signers`, `~/.config/op-env/*`, `~/.local/bin/opwith`, `~/.local/bin/brain` | `home/git/`, `home/op-env/`, `home/bin/` | `home/install.sh` |
| `~/.local/bin/obsidian-sync` (mac only; the mirror it writes under `~/Library/Mobile Documents/iCloud~md~obsidian/` is generated, not owned) | `home/bin/obsidian-sync` | `home/install.sh mac` |
| `~/.local/bin/obsidian-snapshot` (mac only; the archives it writes to `ObsidianVaults/_Snapshots/` in Google Drive are generated, not owned) | `home/bin/obsidian-snapshot` | `home/install.sh mac` |
